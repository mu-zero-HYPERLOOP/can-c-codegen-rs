...
pub fn generate_setup(node_config : &config::NodeRef, network_config : &config::NetworkRef, source : &mut String, header : &mut String, options : &Options) -> Result<()>{
    ...
    source.push_str("#define COMPUTE_BUILD_YEAR \\
    ( \
        (__DATE__[ 7] - '0') * 1000 + \\
        (__DATE__[ 8] - '0') *  100 + \\
        (__DATE__[ 9] - '0') *   10 + \\
...
{indent}__oe_build_time = {{
{indent}{indent}.m_year = BUILD_YEAR,
{indent}{indent}.m_month = BUILD_MONTH,
{indent}{indent}.m_day = BUILD_DAY,
{indent}{indent}.m_hour = BUILD_HOUR,
{indent}{indent}.m_min = BUILD_MIN,
{indent}{indent}.m_sec = BUILD_SEC
{indent}}};
{setup_cans}
