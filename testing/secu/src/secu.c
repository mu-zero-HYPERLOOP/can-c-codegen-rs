...
void canzero_can0_send(canzero_frame *frame) {
  ...
  uint32_t now = time_now_ms();
#ifdef LOGGING
  printf("[%u] secu : can0: sending frame [id=%u, dlc=%u, data=%lu]\n", now, frame->id,
...
int canzero_can0_recv(canzero_frame *frame) {
  ...
  uint32_t now = time_now_ms();
#ifdef LOGGING
  printf("[%u] secu : can0: receiving frame [id=%u, dlc=%u, data=%lu]\n", now, frame->id,
         frame->dlc, *(uint64_t *)frame->data);
#endif
  return 1;
}
...
#ifdef LOGGING
  printf("[%u] secu : request canzero_update at %u\n", now, next_update);
  fflush(stdout);
#endif
}
